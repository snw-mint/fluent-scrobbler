    ...
    public sealed partial class HomePage : Page
    {
        ...
        private void ApplyCachedState()
        {
            ...
            if (_cachedNowPlayingVisible)
            {
                ...
                if (!string.IsNullOrEmpty(_cachedNowPlayingArtUrl))
                {
                    try
                    {
                        var bmp = new BitmapImage { DecodePixelWidth = 160, UriSource = new Uri(_cachedNowPlayingArtUrl) };
        ...
        private async void RefreshButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                if (cur != null)
                {
                    ...
                    await ApplyNowPlayingAsync(cur.Artist, cur.Album, cur.Track, null);
                }
                else
                {
                    SetNowPlayingIdle();
            ...
            {
                LogService.LogError("[HomePage] Refresh failed", ex);
            }
            finally
            {
        ...
        private async Task ApplyNowPlayingAsync(string artist, string album, string track, string? lastFmArtUrl)
        {
            ...
            if (!string.IsNullOrEmpty(artUrl))
            {
                try
                {
                    var bmp = new BitmapImage { DecodePixelWidth = 160, UriSource = new Uri(artUrl) };
                    NowPlayingAlbumArtImage.Source = bmp;
                    NowPlayingAlbumArtImage.Visibility = Visibility.Visible;
                    NowPlayingFallbackIcon.Visibility = Visibility.Collapsed;
                }
            ...
            try
            {
                if (_lastFmService.IsLoggedIn())
                {
                    ...
                    if (!string.IsNullOrEmpty(username))
                    {
                        ...
                        var tracksTodayTask = _lastFmService.GetRecentTracksAsync(username, limit: 200, fromTimestamp: todayUts, forceRefresh: forceRefresh);
